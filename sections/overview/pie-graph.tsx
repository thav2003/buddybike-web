...
export function PieGraph() {
  ...
  React.useEffect(() => {
    // Giả sử dữ liệu employee đã có sẵn hoặc đã được fetch từ API
    const chartConfig = employee.reduce(
      (acc, curr) => {
        // Nếu trường đại học chưa có trong accumulator, tạo mới
        if (!acc[curr.university]) {
          acc[curr.university] = {
            label: curr.university,
            color: `hsl(var(--chart-${Object.keys(acc).length + 1}))` // Tạo màu động cho trường
          };
        }
        return acc;
      },
    ...
    chartConfig['students'] = { label: 'Sinh viên' };
    // Cập nhật chartConfig khi có dữ liệu
    setChartConfig(chartConfig);
  }, []);
  // const chartConfig = employee.reduce((result, e) => {
  //   const { university } = e;
  //   if (!result[university]) {
  //     result[university] = {
  //       label: chartConfig[university]?.label || 'Unknown',
  //       color: chartConfig[university]?.color || 'hsl(var(--default))'
  //     };
  //   }
  //   return result;
  // }, {} as ChartConfig);
      ...
      (acc, curr) => {
        ...
        if (!acc[curr.university]) {
          acc[curr.university] = {
            ...
            students: 0, // Số lượng sinh viên
            fill: '' // Màu sắc (chưa có giá trị)
          };
        }

        // Tăng số lượng sinh viên cho trường hiện tại
        acc[curr.university].students += 1;

        return acc;
      },
      {} as Record<
        string,
        { university: string; students: number; fill: string }
      >
    )
  )
    .map((data, index) => ({
      ...data,
      fill: `hsl(var(--chart-${index + 1}))` // Gán màu sắc cho từng trường
    }))
    .concat({
      university: 'Khác',
      students: employee.filter((e) => !e.university).length,
      fill: `hsl(var(--chart-${employee.length + 1}))`
    });
