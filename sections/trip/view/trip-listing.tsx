...
export default async function TripListingPage() {
  ...
  const mockTrips: Trip[] = [
    ...
    },
    {
      ...
      pickupLocation:
        '68 B\u00e0 Hom, Ph\u01b0\u1eddng 13, Qu\u1eadn 6, TP.HCM',
      destination: '\u0110\u1ea1i h\u1ecdc FPT',
      routeDistance: '26',
      price: '78,000',
      vouchers: '20%',
  ...
  const filteredTripsBySearch = filteredTripsByDate.filter((trip) => {
    ...
    return (
      trip.driverName?.toLowerCase().includes(lowerSearch) ||
      trip.userName?.toLowerCase().includes(lowerSearch) ||
      trip.tripCode?.toLowerCase().includes(lowerSearch)
    );
  });
